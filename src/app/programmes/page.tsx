...
const LEVELS: LeadershipLevel[] = ["Individual contributors", "Frontline", "Middle management", "Senior leaders", "Executives"];
  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Editable. The mapper and account briefs match capability gaps against these programmes."
        actions={<Button variant="primary" onClick={() => setEditing("new")}><Plus size={15} /> Add programme</Button>}
      />
      <p className="mb-4 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-sm text-amber-900">
        Programmes marked <strong>DEMO</strong> are placeholders, not official Bloom programmes. Replace them with Bloom&apos;s real catalogue before using the system with clients.
      </p>
      {db.programmes.length === 0 ? (
        <EmptyState title="No programmes" description="Add Bloom programmes and tag the capabilities each one develops." />
      ) : (
          ...
          {db.programmes.map((p) => (
            ...
            >
              <div className="mb-2 flex flex-wrap gap-1.5">
                {p.isDemo ? <Badge tone="amber">DEMO placeholder</Badge> : <Badge tone="brand">Bloom programme</Badge>}
                <Badge>{p.format}</Badge>
              </div>
              <p className="text-sm text-slate-600">{p.description}</p>
              <p className="mt-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Capabilities</p>
              <div className="mt-1 flex flex-wrap gap-1">{p.capabilities.map((c) => <Badge key={c} tone="brand">{CAPABILITIES[c]}</Badge>)}</div>
...
function ProgrammeForm({ programme, onClose }: { programme?: Programme; onClose: () => void }) {
  ...
  const submit = (e: FormEvent) => {
    ...
    if (f.capabilities.length === 0) return setError("Tag at least one capability so the mapper can match it");
  ...
  return (
    ...
    >
      <form id="prog-form" onSubmit={submit} className="grid gap-3" noValidate>
        ...
        <fieldset>
          <legend className="mb-1 text-xs font-medium text-slate-600">Capabilities developed *</legend>
          <div className="grid gap-1 sm:grid-cols-2">
            {(Object.keys(CAPABILITIES) as CapabilityKey[]).map((c) => (
        ...
        <label className="flex items-center gap-2 text-sm text-slate-700">
          ...
          This is a DEMO placeholder (not an official Bloom programme)
        </label>
      </form>
    </Modal>
  );
